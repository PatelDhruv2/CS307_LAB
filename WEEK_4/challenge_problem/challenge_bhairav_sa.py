...
def fitness(seq):
    ...
    if seq[-1] in (60,72):
        score += 2.0
    if len(seq) > 1 and seq[-1] == 72 and seq[-2] == 71:
        score += 1.0
    repeats = sum(1 for i in range(len(seq)-1) if seq[i]==seq[i+1])
    score -= 0.3 * repeats
    return score

def random_seq(length=20):
...
plt.plot(best, marker='o', linestyle='-', linewidth=1.2)
plt.yticks(SCALE, [NOTE_NAMES[n] for n in SCALE])
plt.title("Generated Melody in Raag Bhairav (Seed 7)")
plt.xlabel("Note Index (Time)")
plt.ylabel("Pitch (Bhairav Scale)")
